...
PTable::PTable(int size)
{
	...
	pcb[0]->SetFileName("../test/nghiala");
	pcb[0]->parentID = -1;
}
...
int PTable::ExecUpdate(char* name)
{
	...
	// 19120447 start
	if( strcmp(name,"../test/nghiala") == 0 || strcmp(name,kernel->currentThread->getName()) == 0 )
	{
	...
	/*
	printf("\nNames of threads in PTable:\n");
	for(int i = 0; i < 10; ++i){
		if(pcb[i] != NULL){
			printf(pcb[i]->GetFileName());
		}
	}
	*/
	return pid;
