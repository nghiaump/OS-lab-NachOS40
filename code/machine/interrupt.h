...
public:
  ...
  PendingInterrupt(IntFunctionPtr func, int param, int time, IntType kind);
  // initialize an interrupt that will
  // occur in the future
  ...
  IntFunctionPtr handler;    // The function (in the hardware device
				// emulator) to call when the interrupt occurs
...
public:
  ...
  void Schedule(IntFunctionPtr handler,           // Schedule an interrupt to occur
